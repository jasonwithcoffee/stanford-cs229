...
def main(train_path, valid_path, save_path):
    ...
    gda_theta = np.concatenate(gda.theta)

    y_pred = gda.predict(x_eval)
    util.plot(x=x_eval, y=y_eval, theta=gda_theta,save_path='gda_image.png')
    # *** END CODE HERE ***
...
class GDA:
    ...
    def fit(self, x, y):
        ...
        self.theta[1] = np.matmul(np.linalg.pinv(sigma),(mu1-mu0))
        self.theta[0] = 0.5*(np.matmul(mu0.T,np.matmul(np.linalg.pinv(sigma),mu0))\
    ...
    def predict(self, x):
        ...
        # *** START CODE HERE ***
        n, d = x.shape
        z = np.matmul(x, self.theta[1]) + np.tile(self.theta[0],(n,1))
        gz = 1/(1 + np.exp(-z))
        return gz.reshape(n,)
        # *** END CODE HERE
